...
export class InitServer {
	...
	setup(config: TServerConfig) {
		...
		this.server.use(cors({
			origin: 'http://localhost:5173', // TODO: config this
			credentials: true
		}));
	...
	async start() {
		...
		try {
			...
			this.server.listen(port, () => console.log(`[server]: ${process.env.NODE_ENV} server is running at ${host}:${port} on process ${process.pid}`));
		} catch (error) {
			console.error(error);
			process.exit(1);
		}
